...
public:
	...
	#if (__cplusplus >= 201103) || (defined (_MSC_VER) && _MSC_VER >= 1800)
	public:
		/* __input_lambda_t: void (aosl_refobj_t iobj, bool free_only) */
		template <typename __input_lambda_t,
			typename std::enable_if<std::is_void<decltype(std::declval<__input_lambda_t>()(
				std::declval<aosl_refobj_t>(),
				std::declval<bool>()))>::value, int>::type = 0>
		int exec (const char *f_name, __input_lambda_t &&input)
		{
			__input_lambda_t *input_obj = new __input_lambda_t (std::move (input));
			int err = aosl_ref_t_oop::exec (f_name, ____input_f<typename std::remove_reference<__input_lambda_t>::type>, 1, input_obj);
			if (err < 0)
				delete input_obj;
	...
	private:
		...
		{
			__input_lambda_t *input_obj = reinterpret_cast<__input_lambda_t *>(argv [0]);
