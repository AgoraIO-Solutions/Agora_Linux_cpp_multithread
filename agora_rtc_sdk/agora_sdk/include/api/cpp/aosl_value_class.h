...
#if (__cplusplus >= 201103) || (defined (_MSC_VER) && _MSC_VER >= 1800)
public:
	/* __verify_lambda_t: void (const aosl_value_t *val) */
	template <typename __verify_lambda_t,
		typename std::enable_if<std::is_same<decltype(std::declval<__verify_lambda_t>()(
			std::declval<const aosl_value_t *>())), int>::value, int>::type = 0>
	static void *verify_register (aosl_kfd_t dfd, const char *name, __verify_lambda_t&& task)
	{
		__verify_lambda_t *task_obj = new __verify_lambda_t (std::move (task));
		void *err = verify_register (dfd, name, ____verify_f<typename std::remove_reference<__verify_lambda_t>::type>,
								____verify_dtor<typename std::remove_reference<__verify_lambda_t>::type>, 1, task_obj);
		if (err == NULL)
			delete task_obj;
...
private:
	...
	{
		__verify_lambda_t *task_obj = reinterpret_cast<__verify_lambda_t *>(argv [0]);
		return (*task_obj) (val);
	}
