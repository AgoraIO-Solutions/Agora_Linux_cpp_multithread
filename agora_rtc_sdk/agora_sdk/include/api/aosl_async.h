...
#define AOSL_STACK_ISOLATED ((const struct _aosl_stack_ *)(uintptr_t)1)
#define aosl_stack_invalid(id) ((id) == AOSL_STACK_INVALID)
#define aosl_stack_isolated(id) ((id) == AOSL_STACK_ISOLATED)
...
/**
 * Start a new isolated coroutine block, which likes the normal
 * async block, but all the async operations inside this kind of
 * block do not affect the outside world, so if we expect some
 * resume node not to wait some async operations, then please
 * put these async operations inside this kind of block.
 **/
extern __aosl_api__ int aosl_isolated_block (aosl_argv_f f, uintptr_t argc, ...);
extern __aosl_api__ int aosl_isolated_block_args (aosl_argv_f f, uintptr_t argc, va_list args);
extern __aosl_api__ int aosl_isolated_block_argv (aosl_argv_f f, uintptr_t argc, uintptr_t argv []);
