...
#define AOSL_PERF_TYPE_MPQ_Q 0x1 /* mpq queue */
#define AOSL_PERF_TYPE_MPQ_C 0x2 /* mpq call */
#define AOSL_PERF_TYPE_TASK_E 0x3 /* task exec */
#define AOSL_PERF_TYPE_TASK_D 0x4 /* task done */
#define AOSL_PERF_TYPE_TASK_F 0x5 /* task free */
#define AOSL_PERF_TYPE_TIMER 0x6 /* timer */
#define AOSL_PERF_TYPE_FD_C 0x7 /* fd check */
#define AOSL_PERF_TYPE_FD_D 0x8 /* fd data */
#define AOSL_PERF_TYPE_FD_E 0x9 /* fd event */
#define AOSL_PERF_TYPE_PREPARE 0xa /* async prepare */
#define AOSL_PERF_TYPE_RESUME 0xb /* async resume */

#define AOSL_PERF_EXI_SENDER_QID 0x01
#define AOSL_PERF_EXI_SENDER_TID 0x02


#define aosl_perf_cb_type(extra_info) (((uint32_t)(extra_info)) >> 24)
#define aosl_perf_cb_info(extra_info) (((uint32_t)(extra_info)) & 0xff)
 ...
 *    none.
 **/
typedef void (*aosl_perf_f_t) (const char *f_name, int free_only, uint32_t wait_us, uint32_t exec_us, uint32_t extra_info, ...);
...
#define aosl_mpq_invalid(mpq) ((intptr_t)(mpq) <= 0)
 ...
 *       ref: the ref object id for indicating whether the relative operation
 *            should be aborted and only free the relative resources, if the
 *            ref object specified by ref has been destroyed, then target
 *            function specified by 'f' MUST NOT DO ANYTING ELSE except free
 *            the relative resources.
 *    f_name: the funciton name
 *         f: the function
 *      argc: the args count
 *       ...: variable args
 * Return value:
...
/**
 * Execute a function with specified args on the specified mpq thread.
 * Parameter:
 *        tq: the target queue id
 *       ref: the ref object id for indicating whether the relative operation
 *            should be aborted and only free the relative resources, if the
 ...
 * Return value:
 *        <0: indicates error, check aosl_errno for detail;
 *         0: async executing issued successfully, resume later;
 *        >0: the function has been executed immediately, resume at once;
 * Remarks:
 *     The function specified by f may be executed in sync or async mode:
 *     1. if the target mpq is just the calling mpq thread, execute it directly,
 *        resume immediately;
 *     2. if the target mpq is not the calling mpq thread, queue it to the target
 *        mpq, and resume later;
 **/
extern __aosl_api__ int aosl_mpq_exec (aosl_mpq_t tq, aosl_ref_t ref, const char *f_name, aosl_mpq_func_argv_t f, uintptr_t argc, ...);
 ...
 *        mpq, and resume later;
 **/
extern __aosl_api__ int aosl_mpq_exec_args (aosl_mpq_t tq, aosl_ref_t ref, const char *f_name, aosl_mpq_func_argv_t f, uintptr_t argc, va_list args);
 ...
 *        mpq, and resume later;
 **/
extern __aosl_api__ int aosl_mpq_exec_argv (aosl_mpq_t tq, aosl_ref_t ref, const char *f_name, aosl_mpq_func_argv_t f, uintptr_t argc, uintptr_t argv []);
...
/**
 * Get the name of the specified mpq
 * Parameters:
 *        qid: the mpq id
 *    namebuf: the buffer for saving the mpq name
 *        len: the buffer length
 * Return value:
 *     <0: indicates error, check aosl_errno for detail;
 *      0: success, the mpq name was saved to namebuf;
 */
extern __aosl_api__ int aosl_mpq_name (aosl_mpq_t qid, void *namebuf, size_t len);
